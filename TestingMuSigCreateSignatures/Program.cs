	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			for (int i = 0; i < peers; i++)
			{
				...
				Console.WriteLine($"Verify signature {i} {musig.Verify(ecPubKeys[i], pubNonces[i], sigs[i])}");
			}
			...
			Console.WriteLine("Verify taprootPubKey: " + taprootPubKey.VerifySignature(msg32, schnorrSig));
