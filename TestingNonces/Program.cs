			...
			using (var nodeBuilder = NodeBuilder.Create(NodeDownloadData.Bitcoin.v25_0, Network.RegTest))
			{
				...
				else
				{
					for (int i = 0; i < spender.Inputs.Count; i++)
					{
						...
						var musig33 = new NBitcoin.Secp256k1.Musig.MusigContext(pairPubKeys_1_to_2, hashScriptSpend.ToBytes());
						...
						musig22.ProcessNonces(pubNonces);
						...
						var sigs22 = new NBitcoin.Secp256k1.Musig.MusigPartialSignature[nonces.Length];
						byte[] backToByptes22;
						...
						for (int n = 0; n < nonces.Length; n++)
						{
							if (n == 0)
							{
								sigs[n] = musig22.Sign(ecPrivateKeys[2], nonces[n]);
								//musig33.Process(pubNonces[n]);
								//Console.WriteLine("verified signature 1 ? " + musig33.Verify(pairPubKeys_1_to_2[n], pubNonces[n], sigs[n]));
							}
							else
							{
								sigs[n] = musig22.Sign(ecPrivateKeys[n], nonces[n]);
								//musig33.Process(pubNonces[n]);
								//Console.WriteLine("verified signature 2 ? " + musig33.Verify(pairPubKeys_1_to_2[n], pubNonces[n], sigs[n]));
							}
							Console.WriteLine("Public signature: " + NBitcoin.DataEncoders.Encoders.Hex.EncodeData(sigs[n].ToBytes()));
							var sigString = NBitcoin.DataEncoders.Encoders.Hex.EncodeData(sigs[n].ToBytes());
							backToByptes22 = NBitcoin.DataEncoders.Encoders.Hex.DecodeData(sigString);
							sigs22[n] = new NBitcoin.Secp256k1.Musig.MusigPartialSignature(backToByptes22);

						}
						// I need to store the "sigs" so I can "aggregate them latter
						signature = musig22.AggregateSignatures(sigs22);
