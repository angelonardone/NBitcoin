...
/*
	...
	"b6f2920002873556366ad9f9a44711e4f34b596a892bd175427071e4064a89cc"
			...
			using (var nodeBuilder = NodeBuilder.Create(NodeDownloadData.Bitcoin.v25_0, Network.RegTest))
			{
				...
				else
				{
					for (int i = 0; i < spender.Inputs.Count; i++)
					{
						...
						//var musig22 = new NBitcoin.Secp256k1.Musig.MusigContext(pairPubKeys_1_to_2, hashScriptSpend.ToBytes());
		...
		static void musig_matching_dc_one_at_time()
		{
			var ctx = Context.Instance;
			var msg32 = Encoders.Hex.DecodeData("502c616d9910774e00edb71f01b951962cc44ec67072757767f3906ff82ebfe8");
			...
			using (var nodeBuilder = NodeBuilder.Create(NodeDownloadData.Bitcoin.v25_0, Network.RegTest))
			{
				...
				else
				{
					for (int i = 0; i < spender.Inputs.Count; i++)
					{
						...
						//var musig22 = new NBitcoin.Secp256k1.Musig.MusigContext(pairPubKeys_1_to_2, hashScriptSpend.ToBytes());
						var musig2 = new NBitcoin.Secp256k1.Musig.MusigContext(pairPubKeys_1_to_2, hashScriptSpend.ToBytes());
						var musig3 = new NBitcoin.Secp256k1.Musig.MusigContext(pairPubKeys_1_to_2, hashScriptSpend.ToBytes());


						//nonces = pairPubKeys_1_to_1.Select(c => musig.GenerateNonce(c)).ToArray();
						// genero 2 nonces usando el public address de cada uno
						nonces = new NBitcoin.Secp256k1.Musig.MusigPrivNonce[1];
						nonces[0] = musig2.GenerateNonce(pairPubKeys_1_to_2[0]);
						Console.WriteLine("public keys: " + NBitcoin.DataEncoders.Encoders.Hex.EncodeData(pairPubKeys_1_to_2[0].ToBytes()));

						var pubNonces = new MusigPubNonce[1];
						var noceBytes = nonces[0].CreatePubNonce().ToBytes();
						pubNonces[0] = new MusigPubNonce(noceBytes);
						Console.WriteLine("one Public nonce 1: " + Encoders.Hex.EncodeData(pubNonces[0].ToBytes()));
						var musig4 = new NBitcoin.Secp256k1.Musig.MusigContext(pairPubKeys_1_to_2, hashScriptSpend.ToBytes());
						var oneNonce = musig4.GenerateNonce(pairPubKeys_1_to_2[0]);
						var oneNoceBytes = oneNonce.CreatePubNonce().ToBytes();
						var onePubNonce = new NBitcoin.Secp256k1.Musig.MusigPubNonce(oneNonce.CreatePubNonce().ToBytes());
						Console.WriteLine("public nonce from var musig4: " + Encoders.Hex.EncodeData(onePubNonce.ToBytes()));
						Console.WriteLine("are the same public nonces? " + onePubNonce.Equals(pubNonces[0]));
						Console.WriteLine("are nonces equals? " + oneNonce.Equals(nonces[0]));


						// I need to store th pubNonces so I can Process all latter
						musig2.ProcessNonces(pubNonces);
						var sigs1 = new NBitcoin.Secp256k1.Musig.MusigPartialSignature[1];


						

						nonces = new NBitcoin.Secp256k1.Musig.MusigPrivNonce[1];
						nonces[0] = musig3.GenerateNonce(pairPubKeys_1_to_2[1]);
						Console.WriteLine("public keys: " + NBitcoin.DataEncoders.Encoders.Hex.EncodeData(pairPubKeys_1_to_2[1].ToBytes()));
						


						//musig.ProcessNonces(nonces.Select(n => n.CreatePubNonce()).ToArray());
						// creo 2 PubNonces usando los nonces creados antes
						pubNonces = new MusigPubNonce[nonces.Length];
						noceBytes = nonces[0].CreatePubNonce().ToBytes();
						pubNonces[0] = new MusigPubNonce(noceBytes);
						Console.WriteLine("one Public nonce 2: " + Encoders.Hex.EncodeData(pubNonces[0].ToBytes()));
						// I need to store th pubNonces so I can Process all latter
						musig3.ProcessNonces(pubNonces);
						var sigs2 = new NBitcoin.Secp256k1.Musig.MusigPartialSignature[1]; 
