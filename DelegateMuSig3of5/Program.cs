...
namespace NBitcoinTraining
{
	class Program
	{
		...
		private static Dictionary<byte[], List<(byte[], TaprootSignature)>> GenerateAndTrackSignatures(
			Key[] privateKeys,
			List<(uint, TapScript)> scriptWeights,
			uint256 hash,
			TaprootSigHash sighash)
		{
			var signatureTracking = new Dictionary<byte[], List<(byte[], TaprootSignature)>>();

			// Step 1: First user (privateKey[0]) signs all combinations they’re part of
			var scriptsForUser0 = lookScriptsToSign(scriptWeights, privateKeys[0]);
			var signaturesUser0 = new List<(byte[], TaprootSignature)>();
			var ecPubKey0 = ECPubKey.Create(privateKeys[0].PubKey.ToBytes());
			var xOnlyPubKey0 = ecPubKey0.ToXOnlyPubKey().ToBytes();

			foreach (var (weight, tapScript) in scriptsForUser0)
			{
				byte[] sigBytes = privateKeys[0].SignTaprootScriptSpend(hash, sighash).ToBytes();
				byte[] finalSigBytes;
				if (sigBytes.Length == 65)
				{
					finalSigBytes = new byte[64];
					Array.Copy(sigBytes, 0, finalSigBytes, 0, 64);
				}
				...
				{
					throw new Exception($"Invalid Schnorr signature size for user 0 (XOnly: {Encoders.Hex.EncodeData(xOnlyPubKey0)}): {sigBytes.Length} bytes");
				}

				var schnorrSig = new NBitcoin.Crypto.SchnorrSignature(finalSigBytes);
				var taprootSig = new TaprootSignature(schnorrSig);
				signaturesUser0.Add((xOnlyPubKey0, taprootSig));
			}

			signatureTracking[xOnlyPubKey0] = signaturesUser0;

			// Step 2: Second user (privateKey[1]) signs subset signed by user 0
			var scriptsForUser1 = lookScriptsToSign(scriptsForUser0, privateKeys[1]);
			var signaturesUser1 = new List<(byte[], TaprootSignature)>();
			var ecPubKey1 = ECPubKey.Create(privateKeys[1].PubKey.ToBytes());
			var xOnlyPubKey1 = ecPubKey1.ToXOnlyPubKey().ToBytes();

			foreach (var (weight, tapScript) in scriptsForUser1)
			{
				byte[] sigBytes = privateKeys[1].SignTaprootScriptSpend(hash, sighash).ToBytes();
				byte[] finalSigBytes;
				if (sigBytes.Length == 65)
				{
					finalSigBytes = new byte[64];
				...
				{
					throw new Exception($"Invalid Schnorr signature size for user 1 (XOnly: {Encoders.Hex.EncodeData(xOnlyPubKey1)}): {sigBytes.Length} bytes");
				}

				var schnorrSig = new NBitcoin.Crypto.SchnorrSignature(finalSigBytes);
				var taprootSig = new TaprootSignature(schnorrSig);
				signaturesUser1.Add((xOnlyPubKey1, taprootSig));
			}

			signatureTracking[xOnlyPubKey1] = signaturesUser1;

			// Step 3: Third user (privateKey[2]) signs subset signed by user 1, resulting in one script
			var scriptsForUser2 = lookScriptsToSign(scriptsForUser1, privateKeys[2]);
			if (scriptsForUser2.Count != 1)
			{
				throw new Exception($"Expected exactly one script for 3-of-5, found {scriptsForUser2.Count}");
			}

			var signaturesUser2 = new List<(byte[], TaprootSignature)>();
			var ecPubKey2 = ECPubKey.Create(privateKeys[2].PubKey.ToBytes());
			var xOnlyPubKey2 = ecPubKey2.ToXOnlyPubKey().ToBytes();

			foreach (var (weight, tapScript) in scriptsForUser2)
			{
				byte[] sigBytes = privateKeys[2].SignTaprootScriptSpend(hash, sighash).ToBytes();
				byte[] finalSigBytes;
				if (sigBytes.Length == 65)
				{
					finalSigBytes = new byte[64];
				...
				{
					throw new Exception($"Invalid Schnorr signature size for user 2 (XOnly: {Encoders.Hex.EncodeData(xOnlyPubKey2)}): {sigBytes.Length} bytes");
				}

				var schnorrSig = new NBitcoin.Crypto.SchnorrSignature(finalSigBytes);
				var taprootSig = new TaprootSignature(schnorrSig);
				signaturesUser2.Add((xOnlyPubKey2, taprootSig));
			}

			signatureTracking[xOnlyPubKey2] = signaturesUser2;
		...
		"6f2b793cc7ecc1f1cac8d7d9d9e13426b5fbf74496ca73a3bacaf025ba34713b"
			...
			var AllTreeInfo = new List<TaprootSpendInfo>();
			AllTreeInfo.Add(treeInfo);
			...
			using (var nodeBuilder = NodeBuilder.Create(NodeDownloadData.Bitcoin.v25_0, Network.RegTest))
			{
				...
				var sighash = TaprootSigHash.All; // Try TaprootSigHash.Default or TaprootSigHash.Single if needed
				...
				else
				{
					for (int i = 0; i < spender.Inputs.Count; i++)
					{
						var scriptExecutionData = new TaprootExecutionData(i, ScriptToSign.LeafHash) { SigHash = sighash };
						var hash = spender.GetSignatureHashTaproot(spentOutputsIn, scriptExecutionData);

						// Extract public keys from the script for debugging
						var scriptOps = ScriptToSign.Script.ToOps();
						var pubKeysInScript = new List<byte[]>();
						foreach (var op in scriptOps)
						{
							if (op.PushData != null && op.PushData.Length == 32) // X-only pubkey size
							{
								pubKeysInScript.Add(op.PushData);
								Console.WriteLine($"Script public key: {Encoders.Hex.EncodeData(op.PushData)}");
							}
						}

						// Generate signatures for each private key and verify
						var signatures = new List<(byte[], TaprootSignature)>();
						for (int j = 0; j < min_num_signatures; j++)
						{
							var privateKey = privateKeys[j];
							var ecPubKey = ECPubKey.Create(privateKey.PubKey.ToBytes());
							var xOnlyPubKey = ecPubKey.ToXOnlyPubKey().ToBytes();
							Console.WriteLine($"Private key {j} XOnlyPubKey: {Encoders.Hex.EncodeData(xOnlyPubKey)}");

							byte[] sigBytes = privateKey.SignTaprootScriptSpend(hash, sighash).ToBytes();
							byte[] finalSigBytes;
							if (sigBytes.Length == 65)
							{
								finalSigBytes = new byte[64];
							...
							{
								throw new Exception($"Invalid Schnorr signature size for private key {j}: {sigBytes.Length} bytes");
							}

							var schnorrSig = new NBitcoin.Crypto.SchnorrSignature(finalSigBytes);
							var taprootSig = new TaprootSignature(schnorrSig);
							signatures.Add((xOnlyPubKey, taprootSig));
							Console.WriteLine($"Signature for XOnlyPubKey {Encoders.Hex.EncodeData(xOnlyPubKey)}: {Encoders.Hex.EncodeData(GetSignatureBytes(taprootSig))}");
						}

						// Verify signatures (if NBitcoin provides a verification method)
						for (int j = 0; j < min_num_signatures; j++)
						{
							var (xOnlyPubKey, taprootSig) = signatures[j];
							var ecPubKey = ECPubKey.Create(xOnlyPubKey); // Directly create ECPubKey from X-only bytes
							byte[] sigBytes = GetSignatureBytes(taprootSig);
							// Note: NBitcoin may not expose direct Schnorr verification; this is a placeholder
							Console.WriteLine($"Verifying signature for XOnlyPubKey {Encoders.Hex.EncodeData(xOnlyPubKey)}...");
							// If NBitcoin provides Schnorr verification, use:
							// var isValid = ecPubKey.VerifySchnorr(hash, sigBytes); // Adjust based on NBitcoin API
							// Console.WriteLine($"Signature valid: {isValid}");
						}

						// Build witness stack: signatures in reverse order of public keys in script
						var ops = new List<NBitcoin.Op>();
						foreach (var pubKey in pubKeysInScript.AsEnumerable().Reverse())
						{
							var signature = signatures.FirstOrDefault(s => s.Item1.SequenceEqual(pubKey));
							if (signature.Item2 == null)
							{
								throw new Exception($"No signature found for public key {Encoders.Hex.EncodeData(pubKey)}");
							}

							byte[] sigBytes = GetSignatureBytes(signature.Item2);
							if (sigBytes.Length != 64)
							{
								throw new Exception($"Invalid Schnorr signature size for public key {Encoders.Hex.EncodeData(pubKey)}: {sigBytes.Length} bytes");
							}
							ops.Add(Op.GetPushOp(sigBytes));
							Console.WriteLine($"Added signature for XOnlyPubKey {Encoders.Hex.EncodeData(pubKey)}: {Encoders.Hex.EncodeData(sigBytes)}");
						}
						...
						spender.Inputs[i].WitScript = new WitScript(ops.ToArray());
						Console.WriteLine($"Witness: {spender.Inputs[i].WitScript}");
					}
				}
		...
		private static byte[] GetSignatureBytes(TaprootSignature sig)
		{
			// Fallback: Parse the string representation as hex (assuming ToString() provides 128-char hex for 64 bytes)
			string sigHex = sig.ToString();
			if (sigHex.Length == 128) // 128 chars = 64 bytes in hex
			{
				return Encoders.Hex.DecodeData(sigHex);
			}
			throw new Exception("Unable to extract bytes from TaprootSignature: Unexpected string format");
		}
